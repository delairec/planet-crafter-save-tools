...
**Une section arrive avec la règle qui la cite.** `WHEN HOLDS_FOR @GAME_RELEASE.2.004`, la dernière release que
`HOLDS_FOR` nomme, porte `INCOMING CONSTRAINED_BY` : une `SECTION` que le jeu écrit aujourd'hui et qu'aucune `RULE` ne nomme sous
...
déclare dans le bloc `WHEN` de cette valeur, et nulle part ailleurs : `INDEX` sous
`WHEN HOLDS_FOR @GAME_RELEASE.2.004`, `LEGACY_INDEX` sous `WHEN HOLDS_FOR @GAME_RELEASE.1.618`, `CONFLICT` et
`RESOLUTION` sous `WHEN DOMAIN merge`. Déclaré au niveau du type puis rendu requis dans le bloc, il resterait légal
partout, la monotonie de `WHEN` ne pouvant rien interdire ; déclaré dans son bloc seul, il est refusé ailleurs par
`L003`. Sous un champ répétable comme `HOLDS_FOR`, il n'est déclaré qu'une fois : une entité qui nomme plusieurs
valeurs entre dans le bloc de chacune
...
qui la distingue, et elle s'ajoute alors à chaque entité qui vaut pour elle. Le code ne discrimine jamais une save
