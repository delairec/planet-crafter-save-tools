...
| **Sessions measured**                           | period A: 56 sessions from tag `before-awawa` (2026-09-11 17:23:31+02:00) to the first write of the refinement work log (2026-09-16 10:19:40 UTC); period B: the 3 sessions of the refinement work; measured by a transcript scan script |
...
| # | Measure                                                   | Period A (56 sessions) | Period B (3 sessions) |
|---|-----------------------------------------------------------|------------------------|-----------------------|
| 1 | User messages typed                                       | 342                    | 17                    |
| 2 | `awawa` commands                                          | 688                    | 54                    |
| 3 | `context` calls vs `show` + `cat`/`sed` on `.awawa` files | 19 vs 249              | 8 vs 9                |
| 4 | `lint` invocations, of which exit ≠ 0                     | 123, 3                 | 7, 3                  |
| 5 | L016 (anchor) findings among all lint findings            | 52 of 93               | 0 of 15               |
| 6 | Share of tool-result characters coming from the corpus    | 41.8 % of 4.13 M       | 15.4 % of 0.47 M      |
| 7 | Share coming from injected skill instructions             | 23.2 %                 | 10.7 %                |
| 8 | Mean length of an assistant text block (characters)       | 1 026                  | 1 136                 |
| 9 | User messages asking for a table, a recap or a correction | 29                     | 3                     |
...
| 2  | Dead entities loaded by every `context` that reaches them                 | 40 closed questions + 6 superseded decisions                                                                                                                    | corpus cleanup branch, 2026-09-15                                      |
...
| 7  | Entities whose subject is the tool, not the project                       | 3 limitations of awawa, 4 rules that had left for the general instructions                                                                                      | corpus audit, 2026-09-16; removed on request three times (14–15 Sept.) |
...
| 10 | `SOURCE` lines citing a usage-report path that no longer exists           | 19                                                                                                                                                              | a pull request moved the report files; nothing reported it             |
| 11 | Fields hiding in `DESC`/`RATIONALE` prose                                 | 21 occurrences of 5 recurring openings                                                                                                                          | corpus audit, 2026-09-16                                               |
| 12 | Cost of settling the target structure                                     | 32 rulings, 14 review steps, 45 named throwaway corpora + 17 lettered variants, 141 probe rows, 9 refused forms, 23 defects, 25 strengths, one day (2026-09-16) | refinement work log, counted by `grep`/`awk`                           |
...
| 3 | A family read from prose and refuted by the tool    | the corpus audit counted 6 facts paying a fictional `REJECTED`; `show` on the six: each has 2–3 substantive `REJECTED` and `SPEC › IMPL` to code — 0 to retype |
...
| 3 | Report paths that rotted silently         | 19 lines to report files moved by one pull request; `FILE anchor` makes that L016                                                                                     |
...
| 2 | Fields hidden in prose, first corpus                                       | `DESC "limitation acceptée, awawa 2.7.0 : …"` ×8, `DESC "garde : …"` ×5, `"la règle vit dans [general instructions] …"` ×4, `DESC "non : …"` ×3, `RATIONALE "rouvrir quand …"` ×1 |
...
Previous usage reports of the project were not read for this synthesis (the retrospective read the refinement work
log and the corpus audit only, by instruction); « times seen » is therefore unchecked except where the project's own
...
|---|------------------------------------------------------|-----------------------------------------------------------------------------------------------------|------------------------------------|-----------------------------------------------------------------------|--------------------------------|------------------------------------------------------------------------------------------------|
| 1 | The corpus read as files instead of through the tool | 8 consecutive usage reports, 2026-09-12 to 2026-09-15, as the user's instructions record + this one | usage report of 2026-09-12, part A | 108 `cat`/`sed` on `.awawa` in period A against 19 `context`          | a written rule                 | no: the rule exists and the ratio stands; the lever is outside the corpus (part A, last table) |
...
Not applicable: this report is the first of its kind and the previous reports were not read (above).

## Appendix

### Measurements

Figures from a scan script over the session transcripts of this project (224 transcripts), read by script only; active
time
...
| # | Claim                                                                                          | Check                                                                                                 | Verdict                                                                                                                                                                                                 |
|---|------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 1 | Corpus audit, finding 3: six `format-save` decisions are facts paying a fictional `REJECTED`   | `show` on the six on 2026-09-16: each carries 2–3 substantive `REJECTED` and `SPEC › IMPL` to code    | wrong                                                                                                                                                                                                   |
| 2 | Corpus audit, finding 1: 137 `DECISION`, 6 superseded; 61 `OPEN_QUESTION`, 21 open / 40 closed | `status DECISION docs` on 2026-09-16: 145 / 12 superseded; `status OPEN_QUESTION docs`: 20 / 41       | overstated by date: the corpus moved between the two readings; the outline recounts at migration                                                                                                        |
...
| 4 | Retrospective scan, period B: 2 throwaway-corpus directories                                   | the design file names 45 corpora `p9`…`p36x` and 17 lettered variants                                 | wrong as a measure of the design cost: the scan's metric matches probe directories named `p*` on the three sessions writing the work log, and the review sessions probed elsewhere or under other paths |
