# What reading the corpus costs the agent's context

Author: Claude Opus 5 (`claude-opus-5`); with medium effort. Out-of-template report,
2026-09-22.
...
from the corpus; the instructions loaded in full weigh more than the corpus did.

## What one session loaded

Perimeter: the session that rewrote `@RULE.APlayerEntryCarriesCameraViewAndItsLifetimeTotals` and
`@RULE.TheGlobalMetadataEntryCarriesLogisticsPaused` (PR #117), 2026-09-22. Line counts are read from the outputs,
not measured in tokens.

| Output                                                                                                                                             | Size                  | What was actually used                                    |
|----------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------|-----------------------------------------------------------|
| `awawa context @RULE.APlayerEntryMayOmitTheFieldsAddedByALaterUpdate --skip reasoning --skip provenance --with-schema .`                           | 145 lines             | about 15; the rest mostly the schema, `SCHEMA *` included |
| `awawa context @RULE.TheGlobalMetadataEntryMayOmitLogisticsPaused --skip reasoning --skip provenance .`                                            | 25 lines              | all of it                                                 |
| `awawa show @TASK.DOCS86 .`                                                                                                                        | about 60 lines        | its `SPEC` lines; `RATIONALE` and `SOURCE` unused         |
| Instructions outside the corpus: `corpus.md`, `../../../agents/corpus-ecrire.md`, `agents/pull-request.md`, `reports.md`, the `awawa-schema` skill | several hundred lines | a small fraction                                          |
...
| Extract one field: `awawa show @TYPE.X --json . \| jq -r '.fields[] \| select(.name=="SPEC") \| .atoms[0]'`                   | only the field asked for                                                                                                                                      | nobody does it by reflex until it is written down                 | CORPUS-4 in `~/.ai/instructions/corpus.md` |
