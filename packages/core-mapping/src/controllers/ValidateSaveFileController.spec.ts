...
describe('ValidateSaveFileController', () => {
  it('should validate a valid save file', async () => {
    ...
    // Assert
    expect(viewModel).toEqual({status: 'valid', errorMessages: [], warnings: []});
  });
  ...
  it('should report the errors of an invalid save file', async () => {
    ...
    // Assert
    expect(viewModel).toEqual({status: 'invalid', errorMessages: ['Expected 11 sections but found 1'], warnings: []});
  });
