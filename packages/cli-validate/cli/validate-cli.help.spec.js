...
describe('Validate CLI help', () => {
  describe('When the help is asked', () => {
    it.each([
      ['Bun', ['bun', VALIDATE_CLI_PATH, '--help']],
      ['Node', ['node', '--import', NODE_LOADER_PATH, VALIDATE_CLI_PATH, '--platform=node', '--help']]
      ...
      // Assert
      expect(cliRun).toEqual({exitCode: 0, stdout: `${VALIDATE_CLI_HELP}\n`, stderr: ''});
    });
  });

  describe('When an argument names no flag the command accepts', () => {
    it.each([
      ['Bun', ['bun', VALIDATE_CLI_PATH, '--unknown']],
      ['Node', ['node', '--import', NODE_LOADER_PATH, VALIDATE_CLI_PATH, '--platform=node', '--unknown']]
